...
export default function PublicationsForm() {
  ...
  return (
    <form className={`${styles["container"]}`}>
      <h1 className={`${styles["header"]}`}>Añadir una nueva Publicación</h1>
      ...
      <select className={`${styles["select"]}`} name='status' id='status'>
        <option value='finished'>Novedades</option>
        <option value='in progress'>Comunidades</option>
      </select>

      <label htmlFor='author'>Autor</label>
      <input className='text-input' id='author' type='text' name='author' />

      <label htmlFor='url-author'>{`Autor URL (opcional)`}</label>
      <input
